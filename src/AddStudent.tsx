...
const AddStudent: React.FC = () => {
  ...
  // bikin QR canvas besar untuk download
  const createLargeQRCanvas = (
    value: string,
    size = 800
    ...
    return new Promise((resolve) => {
      ...
      // kasih delay dikit biar QR kebentuk
      setTimeout(() => {
  ...
  // helper download PNG
  const downloadQRValue = async (value: string, filename: string) => {
    const qrCanvas = await createLargeQRCanvas(value, 800); // ukuran besar
  ...
  const handleDownloadQR = (student?: Student) => {
    ...
    downloadQRValue(
      ...
      `QR_${target.id}_${target.name}.png`
    );
  };
  ...
  const handleDownloadSelectedQR = async () => {
    ...
    for (const id of selectedIds) {
      ...
      const qrCanvas = await createLargeQRCanvas(JSON.stringify(s), 800);
